...
{
    if (v_galaxy_generation[gal_id].i_star > 0)
    {
        if (edit == false)
        {
        ...
        v_galaxy_generation[gal_id].density = (v_galaxy_generation[gal_id].i_gsize / 1.0) / (v_galaxy_generation[gal_id].i_star / 1.0);
        int bythe = v_galaxy_generation[gal_id].i_gsize, temp_edit = 0;
        int max = 0, dist = 8, limit = 0;
        bool is_ok = true;
...
void c_init::initilizers()
{
    ...
    for (int i = 0; i < v_galaxy_generation.size(); i++)
    {
        ...
        dist = v_galaxy_generation[i].density * 50;
        fallen_dist = v_galaxy_generation[i].density * 150;
        leviathan_dist = v_galaxy_generation[i].density * 75;
        marauder_dist = v_galaxy_generation[i].density * 150;
        if (dist < 30)
        {
            dist = 30;
        }
        ...
        if (db.debug_initializers)
        {
            std::cout << "Density is: " << v_galaxy_generation[i].density << " :distance is " << dist << " fallen distance " << fallen_dist << " leviathan dist " << leviathan_dist << " marauder distance "
                << marauder_dist << std::endl;
        }
        // Empire Generation
        ...
        for (int f = 0; f < gal_empire_am; f++)
        {
            if (db.debug_initializers)
            {
                std::cout << "Empire nmber " << f << std::endl;
            }
            placed = true;
            min_together(i);
...
void c_init::min_together(int gal_id)
{
    ...
    if (db.debug_initializers)
    {
        std::cout << "Random system id " << random_sys << std::endl;
    }
    while ((v_system_data[gal_id][random_sys].inicjalizer == true) && (!everything_filled))
    {
        random_sys = rand() % v_galaxy_generation[gal_id].i_star;
        if (limit > 10000)
        {
...
void c_init::min_dist(int gal_id)
{
    ...
    while ((placed == true) && (end == false))
    {
        for (int j = 0; j < v_galaxy_generation[gal_id].i_star; j++)
        {
            if (j != random_sys)
            {
                if(sqrt(pow(v_system_data[gal_id][random_sys].gal_x - v_system_data[gal_id][j].gal_x, 2) + 
                    pow(v_system_data[gal_id][random_sys].gal_y - v_system_data[gal_id][j].gal_y, 2)) < dist)
                {
                    if (v_system_data[gal_id][j].inicjalizer == true)
                    {
                        if ((v_system_data[gal_id][j].init_type != 0) && (v_system_data[gal_id][j].init_type != 99))
                        {
                            placed = false;
                            break;
                        }
...
void c_init::min_dist_specific(int gal_id, int min_distance, int init_id)
{
    ...
    while ((specific_init == true) && (end == false))
    {
        ...
        {
            if (sqrt(pow(v_system_data[gal_id][random_sys].gal_x - v_system_data[gal_id][j].gal_x, 2) + 
                pow(v_system_data[gal_id][random_sys].gal_y - v_system_data[gal_id][j].gal_y, 2) < min_distance))
            {
                if (v_system_data[gal_id][j].inicjalizer == true)
                {
